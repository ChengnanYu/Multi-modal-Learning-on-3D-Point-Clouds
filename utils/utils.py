...
def plot_loss_curve(history,n_points_avg,n_points_plot,save_dir):
    ...
    plt.close()
'''    
def plot_loss_curve(total_loss,save_dir):
    plt.plot(total_loss, '-g',label = 'train_loss')
    plt.legend()
    plt.ylabel('loss')
    plt.xlabel('iteration')
    plt.title('Training loss')
    plt.savefig(save_dir + '/'+ str(datetime.datetime.now().strftime('%Y-%m-%d %H-%M')) + '_total_loss.png')
    plt.close()
